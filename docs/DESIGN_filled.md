...
두 진영 문서를 같은 비중으로 찾아야 하는 과제라서, 약한 쪽의 성능을 보는 min(SW, HW) 기준을 먼저 확인했다. Qwen3는 SW 질문 MRR(0.770)이 bge-m3(0.748)보다 조금 높았지만 HW 질문에서 0.493으로 떨어져, 이 기준에서 bge-m3(0.635~0.659)보다 낮았다. 진영 간 균형 조건을 충족하지 못한 것이다. 이어서 전체 지표를 비교한 결과, 두 측정 조건 모두에서 bge-m3의 Hit@1, Hit@5, MRR이 가장 높아 이 기준을 충족한다는 점을 다시 확인했다. 설계 초안에서는 Qwen3를 가정했지만 이 결과에 따라 bge-m3로 바꿨다.
