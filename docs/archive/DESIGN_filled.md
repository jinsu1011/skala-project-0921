...
- 기술은 조가 직접 골랐다(2안). 조가 후보 6개를 같은 기준표로 채점해 두 기술을 정했고, 에이전트는 선정을 바꾸지 않고 원문 근거로 검증한 결과만 기록한다.
...
SW / HW MRR은 구현 조건 값이고, 평가 조건에서는 bge-m3 0.748 / 0.659, Qwen3 0.770 / 0.493이다(outputs/eval/embedding_eval.csv).
...
![그림 1. 임베딩 후보(a)와 검색 구성(b)의 측정 결과(구현 조건). 출처: 조 자체 측정(42문항 평가셋), eval/run_embedding_eval.py로 측정하고 report/plot_eval_figure.py로 작성](../outputs/eval/embedding_eval_ko.png)<!--img:15.0-->

두 진영 문서를 같은 비중으로 찾아야 하는 과제라서, 약한 쪽의 성능을 보는 min(SW, HW) 기준을 먼저 확인했다. Qwen3는 SW 질문 MRR(0.770)이 bge-m3(0.748)보다 조금 높았지만 HW 질문에서 0.493으로 떨어져(두 조건 같음), 이 기준에서 bge-m3(평가 0.659, 구현 0.635)보다 낮았다. 진영 간 균형 조건을 충족하지 못한 것이다. 이어서 전체 지표를 비교한 결과, 두 측정 조건 모두에서 bge-m3의 Hit@1, Hit@5, MRR이 가장 높아 이 기준을 충족한다는 점을 다시 확인했다. 설계 초안에서는 Qwen3를 가정했지만 이 결과에 따라 bge-m3로 바꿨다.
...
flowchart TB
    ...
    subgraph S1["1. 선정 검증 및 RAG 준비"]
        INIT["initialize<br/>State 초기화"] --> IDX["index_builder<br/>PDF 로드·청킹·인덱스"]
        IDX --> VAL["selection_validator<br/>선정 타당성 검증·기록"]
        VAL --> QP["query_planner<br/>기술별 개요 질의"]
